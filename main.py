...
from engine.engine import (
    HandRecognition, GestureRecognition, GestureEnums, 
    UserStatusEnums, GameEngine
)
...
def handle_image_navigation(data):
    ...
    try:
        ...
        if (
            landmark.index_finger.finger_tip[0] is not None 
            and landmark.thumb.finger_tip[0] is not None
        ):
            cursor = gesture_recognizer.calculate_middle_point(
                landmark.index_finger.finger_tip, landmark.thumb.finger_tip
            )
        ...
        emit('response', {
            "status": "ok", "cursor": cursor, "click": click, 
...
def handle_image(data):
    ...
    try:
        landmark, image = hand_recognizer.getLandmark(input_img)
        recognition_game_status = [UserStatusEnums.CONNECTED, 
                                   UserStatusEnums.PLAYING]
